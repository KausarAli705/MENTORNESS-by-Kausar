...
select AVG(ld.Kill_Count) AS Avg_Kill_Count, pd.L1_Code
FROM pd
JOIN ld
ON pd.P_ID = ld.P_ID
...
select pd.P_ID, COUNT(DISTINCT(ld.Start_datetime)) as unique_dates
from pd
join ld on pd.P_ID = ld.P_ID
group by pd.P_ID
...
SELECT pd.P_ID, ld.level as level, sum(ld.Kill_Count) AS Total_Kill_Counts, avg(ld.Kill_Count) as AVG_Kill_Counts
FROM pd
JOIN ld ON pd.P_ID = ld.P_ID
where Difficulty="Medium"
group by pd.P_ID, level
having Total_Kill_Counts>AVG_Kill_Counts
order by level asc;
...
(SELECT Dev_Id, min(start_datetime) as first_login_datetime 
from ld
group by Dev_Id) as mini_date
on ld.Dev_Id=mini_date.Dev_Id and ld.start_datetime=mini_date.first_login_datetime
order by ld.P_ID;
